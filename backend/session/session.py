...
class SessionFactory:
    ...
    def get_session_obj(handler):
        ...
        if config.SESSION_TYPE == "cache":           #根据不用的配置设置session的存储方式
...
class CacheSession:
    session_container = {}
    session_id = "__sessionId__"   #有几个含义，对应全局的一个session，也是cookie的名字

    def __init__(self, handler):
        self.handler = handler
        client_random_str = handler.get_cookie(CacheSession.session_id, None)           #创建session时，如果有cookie直接读取cookie
        if client_random_str and client_random_str in CacheSession.session_container:   #从内存中根据cookie读取session，正确的才说明是正确的session
            self.random_str = client_random_str
        else:
            self.random_str = create_session_id()    #创建的随机字符串，即cookie的对应值
            CacheSession.session_container[self.random_str] = {}

        expires_time = time.time() + config.SESSION_EXPIRES
        handler.set_cookie(CacheSession.session_id, self.random_str, expires=expires_time)   #设置cookie

    def __getitem__(self, key):         #采用索引的方式设置对象的值，读取值，删除值，方便使用字符串
        ret = CacheSession.session_container[self.random_str].get(key, None)
        return ret

    def __setitem__(self, key, value):
        CacheSession.session_container[self.random_str][key] = value      #在内存中设置此session的字典
