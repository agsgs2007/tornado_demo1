...
class Field:
    ...
    def match(self, name, value):
        ...
        if not self.required:     #可以为空--》通过
            self.is_valid = True
            self.value = value
        else:                       #必需
            if not value:        #发过来的值为空
                if self.custom_error_dict.get('required', None):         #自定义错误提示词典
...
class StringField(Field):
    ...
    def __init__(self, custom_error_dict=None, required=True):
        ...
        if custom_error_dict:
            self.custom_error_dict.update(custom_error_dict)      #更新错误提示词典

        self.required = required      #默认要求为必需
